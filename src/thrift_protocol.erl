...
write_safe(Proto, {Type, Data}) ->
    try
        {write_frag(Proto, Type, Data), ok}
    catch
        {?MODULE, ProtoError} -> ProtoError
    end;
write_safe(Proto, Data) ->
    try
        {write_frag(Proto, Data), ok}
    catch
        {?MODULE, ProtoError} -> ProtoError
    end.
...
write_frag(Proto0, {struct, _, StructDef}, Data, StructName) when
    is_list(StructDef), is_tuple(Data)
->
    Proto1 = write_frag(Proto0, #protocol_struct_begin{name = StructName}),
...
write_frag(Proto0, {struct, union, StructDef} = Type, {_Name, _Value} = Data) when
    is_list(StructDef)
->
