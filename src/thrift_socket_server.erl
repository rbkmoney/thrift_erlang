    ...
    ServiceModuleMap =
        case State#thrift_socket_server.service of
            undefined ->
                lists:foldl(
                    fun
                        ({ServiceName, Service}, Acc) when is_list(ServiceName), is_tuple(Service) ->
...
acceptor_loop({Server, Listen, Service, Handler, SocketOpts, Framed, SslTransport, SslOptions, Proto}) when
